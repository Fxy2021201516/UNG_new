...
#     --dataset $DATASET > words_build_index_output.txt 2>&1


# # 转换words_query数据格式
# for ((i=1; i<=$NUM_QUERY_SETS; i++))
# do
#     INPUT_FILE="$DATA_DIR/words/words_query/words_query.fvecs"
#     OUTPUT_FILE="$DATA_DIR/words/words_query/words_query.bin"
...
# for ((i=1; i<=$NUM_QUERY_SETS; i++))
# do
#     QUERY_DIR="$DATA_DIR/words/words_query"
...
do    
    ...
    # 创建当前查询集的结果目录
    CURRENT_RESULT_DIR="$RESULT_DIR/words_query"
    ...
    ./build_words/apps/search_UNG_index \
        ...
        --num_entry_points 16 \
        --Lsearch 50 300 500 1000 1200 3000 3500 4000 4500 5000 5500 6000 > words_search_output.txt 2>&1
    # 检查执行状态
    if [ $? -eq 0 ]; then
